...
void bn_shift_iadd( bn_t * r,uint32_t i, int shift) {
    ...
    if (bit_offset) {
        ...
        bn_shift_iadd(r,small_part,word_offset * 32);
        bn_shift_iadd(r,large_part,( word_offset+1 ) * 32);
    } else {
        uint32_t zero = 0;
        uint32_t w; // word
        int overflow = 0;
